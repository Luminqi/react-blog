...
interface Props {
  ...
  visibility: boolean
  toolList: { [key: string]: 'able' | 'disable' } | null
    ...
    <Popover position={position} visibility={visibility}>
      {showInput 
        ...
        : <InlineToolSet toolList={toolList}>
            <Bold active={!!activeStyle && activeStyle.includes('BOLD')} disabled={toolList.BOLD === 'disable'} changeInlineStyle={changeInlineStyle} />
            <Italic active={!!activeStyle && activeStyle.includes('ITALIC')} disabled={toolList.ITALIC === 'disable'} changeInlineStyle={changeInlineStyle} />
...
function InlineToolSet ({
  ...
  toolList: { [key: string]: 'able' | 'disable' }
  children: JSX.Element[]
}): JSX.Element {
  return (
    <div className="inlineToolSet" {...props}>
      {children.filter(element => {
        ...
        return toolList[styleName] || (name === 'Separator' && toolList.H3)
      })}
    </div>
  )
}
...
function Bold ({
  ...
  const status = disabled ? 'disabled' : active ? 'activated' : 'unactivated'
  const handleBoldClick = useCallback(() => {
    if (!disabled)
    changeInlineStyle('BOLD')
  }, [disabled])
  return (
    <button className={`inline-btn bold ${status}`} onClick={handleBoldClick} {...props}>
...
function Italic ({
  ...
  return (
    <button className={`inline-btn italic ${status}`} onClick={handleItalicClick} {...props}>
